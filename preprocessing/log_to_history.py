"""
[2025-04-10]: removed /multioutput
[2025-05-09]: added os.path.exists(dir_out) in __serialize_object
...
class Log():
    def __init__(self, log, setting, log_cols):
        self.__log_name = log
        if len(log_cols) > 0:
            self.__log = pd.read_csv('event_log/'+log+'.csv', usecols=log_cols)
        else:
        ...
        print("Columns in event log:")
        print(self.__log.columns.to_list())
        print()

        print("Missing values in the event log")
        missing_values = self.__log.isnull().sum()
        print(missing_values)
        print()

        print("Case count in event log:")
        print(self.__log['case'].nunique())
        print()
    ...
    def get_event_log(self):
        """
        Returns the event log dataframe.
        """
        return self.__log
    ...
    def __split_log(self):
        print(">> Performing __split_log")

        # Added by 2025-05-09: empty values control
        columns_with_none = self.__log.columns[self.__log.isnull().any()].tolist()
        ...
        for v in self.__dict_label_train:
            if v!='timesincecasestart':
                ...
                if None in temp_list:
                    print(f"ERROR! Column '{v}' contains None values in temp_list.")
                    # print("temp_list:", temp_list) # debug
        ...
        print(">> Serializing with __serialize_object")
    ...
    def __serialize_object(self, lista, type):
        print(">>> __serialize_object")
        path_out = 'multioutput/log_history/'+self.__log_name+'/'+self.__log_name+'_'+type+'_'+self.__setting+'.pkl'
